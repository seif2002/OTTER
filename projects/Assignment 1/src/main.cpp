...
std::string windowTitle = "100779102 - Emmanuel Sih";
...
int main() {
	...
	std::cout << "SPACE BAR to switch cameras";
	...
	camera2->SetOrthoVerticalScale(5.0f);
	...
	//	transform2 = glm::rotate(glm::mat4(1.0f), -static_cast<float>(thisFrame), glm::vec3(0, 0, 1)) * glm::translate(glm::mat4(1.0f), glm::vec3(0, 0.0f, glm::sin(static_cast<float>(thisFrame))));
		...
		{
			shader->SetUniformMatrix("u_ModelViewProjection", camera2->GetViewProjection() * transform4);
			vao4->Draw();
		}
